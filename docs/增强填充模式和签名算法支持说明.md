...
> 说明
>
> 本文档描述的是算法层能力与扩展方向。当前 `CryptoTool.Win` 主界面中的 RSA 加密只暴露 `PKCS1` 和 `OAEP`，不再提供 `NoPadding/None` 选项；SM2 页面中的密文格式与签名格式选择已经接入实际加解密和签名验签流程。
...
- **None**: 预留枚举值；当前算法实现中的RSA加密未启用该模式，WinForms界面也不提供该选项
