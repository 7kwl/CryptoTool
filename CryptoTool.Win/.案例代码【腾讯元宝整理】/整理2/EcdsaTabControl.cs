...
namespace CryptoTool.Win
{
    public partial class EcdsaTabControl : UserControl
    {
        ...
        /// <summary>曲线类别切换 → 联动刷新曲线下拉框。</summary>
        private void ComboEcdhCategory_SelectedIndexChanged(object? sender, EventArgs e)
        {
            if (sender == null || comboEcdhCategory.SelectedItem == null) return;
            dynamic selectedItem = comboEcdhCategory.SelectedItem;
            string categoryKey = selectedItem.Value;
            if (!_allCurveData.TryGetValue(categoryKey, out var categoryData)) return;
            comboEcdhCurve.Items.Clear();
            foreach (var c in categoryData.Curves) comboEcdhCurve.Items.Add(c);
            if (comboEcdhCurve.Items.Count > 0) comboEcdhCurve.SelectedIndex = 0;
        }

        /// <summary>获取当前选中的 ECDH 曲线名。</summary>
        private string GetEcdhSelectedCurve()
            => (comboEcdhCurve.SelectedItem is KeyValuePair<string, string> sel && !string.IsNullOrEmpty(sel.Key))
                ? sel.Key : "prime256v1";

        /// <summary>生成 Alice/Bob 密钥对 + 派生共享密钥。</summary>
        ...
        /// <summary>ECDH 加密（ECIES 或 8gwifi.org 静态模式）。</summary>
        private void BtnEcdhEncrypt_Click(object? sender, EventArgs e)
        {
            try
            {
                ...
                if (!string.IsNullOrWhiteSpace(textEcdhIV.Text))
                {
                    ...
                    { AppendValidationResult($"❌ IV 格式无效: {ex.Message}", Color.Red); SetStatus("IV 格式错误"); return; }
                }
                ...
                {
                    // 8gwifi.org：静态 ECDH，需要 Alice 私钥 + Bob 公钥
        ...
        private void BtnEcdhDecrypt_Click(object? sender, EventArgs e)
        {
            try
            {
                ...
                { AppendValidationResult("⚠️ 请先加密生成密文，或将 Base64 密文粘贴到密文框", Color.Orange); SetStatus("无密文输入"); return; }

                var mode = GetEcdhMode();
                string curve = GetEcdhSelectedCurve();
                byte[] plain; byte[] shared; byte[] iv;

                if (mode != EcdhMode.GwifiOrg)
                {
                    // ECIES 解密：需要接收方(Bob)私钥
                    ...
                    byte[] combined;
                    try { combined = Convert.FromBase64String(cipherSource); }
                    catch (FormatException)
                    { AppendValidationResult("❌ 密文格式无效: 请输入 Base64 编码的密文", Color.Red); SetStatus("Base64 格式错误"); return; }
                    try { plain = EcdhAlgorithm.EciesDecrypt(combined, bobPriv, curve, mode, out shared, out iv); }
                    catch (InvalidCipherTextException)
                    { AppendValidationResult("❌ ECIES 解密失败: MAC 校验不通过", Color.Red); SetStatus("ECIES 解密失败"); return; }
                    textEcdhSharedKey.Text = Convert.ToBase64String(shared);
                    textEcdhIV.Text = EcdhAlgorithm.FormatIv(iv, mode);
                    _ecdhLastIV = iv;
                ...
                {
                    // 8gwifi.org 解密：需要 Bob 私钥 + Alice 公钥
                    if (string.IsNullOrWhiteSpace(textEcdhBobPrivate.Text) || string.IsNullOrWhiteSpace(textEcdhAlicePublic.Text))
                    { MessageBox.Show("8gwifi.org 模式解密需要 Bob 私钥和 Alice 公钥", "提示", MessageBoxButtons.OK, MessageBoxIcon.Warning); return; }
                    var bobPriv = EcdsaKeyHelper.ImportPrivateKeyPem(textEcdhBobPrivate.Text.Trim());
                    var alicePub = EcdsaKeyHelper.ImportPublicKeyPem(textEcdhAlicePublic.Text.Trim());
                    try { plain = EcdhAlgorithm.StaticEcdhDecrypt(cipherSource, bobPriv, alicePub, mode, out shared, out iv); }
                    catch (InvalidCipherTextException)
                    { AppendValidationResult("❌ 8gwifi.org 解密失败: MAC 校验不通过", Color.Red); SetStatus("8gwifi.org 解密失败"); return; }
                    textEcdhSharedKey.Text = Convert.ToBase64String(shared);
                    textEcdhIV.Text = EcdhAlgorithm.FormatIv(iv, mode);
                    _ecdhLastIV = iv;
        ...
        /// <summary>将 UI 下拉项映射为 EcdhMode 枚举。</summary>
        ...
        /// <summary>获取 ECDH 文本编码（UTF-8 / GBK / Unicode）。</summary>
        ...
        /// <summary>尝试将 Hex 字符串解析为字节数组。</summary>
        private static bool TryParseHex(string? input, out byte[] bytes)
        {
            bytes = [];
            if (string.IsNullOrWhiteSpace(input)) return false;
            try { bytes = Convert.FromHexString(input.Trim().Replace("0x", "").Replace("0X", "").Replace(" ", "")); return bytes.Length > 0; }
            catch { return false; }
        }
        ...
        {
            if (Clipboard.ContainsText()) { textEcdhOutput.Text = Clipboard.GetText().Trim(); SetStatus("密文已粘贴到 ECDH 密文框"); }
        }

        private void BtnEcdhClear_Click(object? sender, EventArgs e)
        {
            textEcdhInput.Clear(); textEcdhOutput.Clear(); textEcdhSharedKey.Clear(); textEcdhIV.Clear();
            _ecdhLastIV = null;
        ...
        /// <summary>初始化所有下拉框默认选项和曲线数据。</summary>
        private void InitializeDefaults()
        {
            ...
            foreach (var cat in _allCurveData)
                comboCategory.Items.Add(new { Text = $"{cat.Value.Icon} {cat.Key}", Value = cat.Key });
            ...
            if (comboEncMode.Items.Count > 0) comboEncMode.SelectedIndex = 0;
            if (comboEncInputFormat.Items.Count > 0) comboEncInputFormat.SelectedIndex = 0;
            if (comboEncOutputFormat.Items.Count > 0) comboEncOutputFormat.SelectedIndex = 0;
        ...
        /// <summary>密钥管理区曲线类别切换 → 联动曲线下拉框。</summary>
        private void ComboCategory_SelectedIndexChanged(object? sender, EventArgs e)
        {
            if (sender == null || comboCategory.SelectedItem == null) return;
            dynamic selectedItem = comboCategory.SelectedItem;
            string categoryKey = selectedItem.Value;
            if (!_allCurveData.TryGetValue(categoryKey, out var categoryData)) return;
            comboCurve.Items.Clear();
            foreach (var c in categoryData.Curves) comboCurve.Items.Add(c);
            if (comboCurve.Items.Count > 0) comboCurve.SelectedIndex = 0;
        }

        /// <summary>获取当前选中的椭圆曲线名。</summary>
        private string GetSelectedCurve()
            => (comboCurve.SelectedItem is KeyValuePair<string, string> sel && !string.IsNullOrEmpty(sel.Key))
                ? sel.Key : "prime256v1";

        /// <summary>加密区曲线类别切换 → 联动曲线下拉框。</summary>
        private void ComboEncCurveCategory_SelectedIndexChanged(object? sender, EventArgs e)
        {
            if (sender == null || comboEncCurveCategory.SelectedItem == null) return;
            dynamic selectedItem = comboEncCurveCategory.SelectedItem;
            string categoryKey = selectedItem.Value;
            if (!_allCurveData.TryGetValue(categoryKey, out var categoryData)) return;
            comboEncCurve.Items.Clear();
            foreach (var c in categoryData.Curves) comboEncCurve.Items.Add(c);
            if (comboEncCurve.Items.Count > 0) comboEncCurve.SelectedIndex = 0;
        }
        ...
        private void BtnGetPublicKeyFromPrivate_Click(object? sender, EventArgs e)
        {
            try
            {
                ...
                { AppendKeyResult("❌ 私钥为空，无法提取公钥", Color.Red); SetStatus("私钥为空，无法提取公钥"); return; }
                SetStatus("正在从私钥提取公钥...");
                var priv = EcdsaKeyHelper.ImportPrivateKeyPem(textPrivateKey.Text.Trim());
                string curveName = EcdsaKeyHelper.GetCurveName(priv);
                if (curveName == "未知曲线")
                { AppendKeyResult("❌ 私钥生成逻辑非法，不符合标准 OpenSSL", Color.Red); SetStatus("私钥曲线无法识别"); return; }
        ...
        private void BtnGetCurveType_Click(object? sender, EventArgs e)
        {
            ...
            {
                if (string.IsNullOrWhiteSpace(textPrivateKey.Text))
                { AppendKeyResult("❌ 私钥为空，无法获取曲线类型", Color.Red); SetStatus("获取曲线类型失败：私钥为空"); return; }
        ...
        private void BtnValidateKeyPair_Click(object? sender, EventArgs e)
        {
            try
            {
                ...
                { SetGenerateResult(GetSelectedCurve(), "❌ 请先生成或导入密钥对"); return; }
                ...
                signer.Init(true, priv); signer.BlockUpdate(testData, 0, testData.Length);
                byte[] signature = signer.GenerateSignature();
                ISigner verifier = SignerUtilities.GetSigner(signerAlg);
                verifier.Init(false, pub); verifier.BlockUpdate(testData, 0, testData.Length);
                bool isValid = verifier.VerifySignature(signature);
                SetGenerateResult(GetSelectedCurve(), isValid ? $"密钥对完全匹配（{testHash}）" : "❌ 密钥对不匹配");
                SetStatus(isValid ? "密钥对验证完成 - 完全匹配" : "密钥对验证完成 - 不匹配");
            }
            catch (Exception ex) { SetGenerateResult(GetSelectedCurve(), $"❌ 验证异常: {ex.Message}"); SetStatus("验证密钥对失败"); }
        }
        ...
        private void BtnSign_Click(object? sender, EventArgs e)
        {
            try
            {
                if (string.IsNullOrWhiteSpace(textPrivateKey.Text)) { SetSignResult("❌ 私钥为空，无法签名", Color.Red); return; }
                if (string.IsNullOrWhiteSpace(textPublicKey.Text)) { SetSignResult("❌ 公钥为空，无法签名", Color.Red); return; }
                if (string.IsNullOrWhiteSpace(textPlainData.Text)) { SetSignResult("❌ 原始数据为空，无法签名", Color.Red); return; }
                ...
                if (!pv.VerifySignature(psig)) { SetSignResult("❌ 密钥对不匹配，无法签名", Color.Red); SetStatus("签名失败 - 密钥对不匹配"); return; }
                // 正式签名
            ...
            catch (Exception ex) { SetSignResult($"❌ 签名失败: {ex.Message}", Color.Red); SetStatus("签名失败"); }
        }
        ...
        private void BtnVerify_Click(object? sender, EventArgs e)
        {
            try
            {
                if (string.IsNullOrWhiteSpace(textPublicKey.Text)) { SetSignResult("❌ 公钥为空，无法验签", Color.Red); return; }
                ...
                s.Init(false, pub); s.BlockUpdate(Encoding.UTF8.GetBytes(textPlainData.Text), 0, Encoding.UTF8.GetByteCount(textPlainData.Text));
                SetSignResult(s.VerifySignature(sig) ? $"签名验证通过（{hashAlg}）" : "❌ 签名验证失败", s.VerifySignature(sig) ? Color.Green : Color.Red);
            }
            catch (Exception ex) { SetSignResult($"❌ 验签异常: {ex.Message}", Color.Red); }
        }
        ...
        private void BtnConvertKey_Click(object? sender, EventArgs e)
        {
            try
            {
                ...
                targetBox.Text = FormatConversionHelper.ConvertStringFormat(input, FormatConversionHelper.ParseInputFormat(inputFormat), FormatConversionHelper.ParseOutputFormat(tf));
                if (radioPrivateKey.Checked) _privateKeyPem = targetBox.Text; else _publicKeyPem = targetBox.Text;
                AppendValidationResult("格式已转换", Color.Gray);
                SetStatus($"格式转换完成 - {inputFormat} → {tf}");
            }
            catch (Exception ex) { MessageBox.Show($"格式转换失败：{ex.Message}", "错误", MessageBoxButtons.OK, MessageBoxIcon.Error); SetStatus("格式转换失败"); }
        }

        /// <summary>自动检测密钥字符串格式（PEM / Hex / Base64）。</summary>
        private static string DetectKeyFormat(string input)
        {
            if (input.Contains("BEGIN", StringComparison.OrdinalIgnoreCase) || input.Contains("END", StringComparison.OrdinalIgnoreCase)) return "PEM";
        ...
        private void ImportKey(bool isPrivate)
        {
            try
            {
                ...
                if (isPrivate) EcdsaKeyHelper.ImportPrivateKeyPem(c); else EcdsaKeyHelper.ImportPublicKeyPem(c);
        ...
        private void BtnCopySignature_Click(object? s, EventArgs e) => TrySetClipboardText(textSignature.Text, "签名已复制到剪贴板");
        private void BtnCopyPlainData_Click(object? s, EventArgs e) => TrySetClipboardText(textPlainData.Text, "原始数据已复制到剪贴板");
        ...
        private void BtnClearPlainData_Click(object? s, EventArgs e) { textPlainData.Clear(); SetStatus("原始数据已清空"); }

        private void BtnCopySignatureData_Click(object? s, EventArgs e) => TrySetClipboardText(textSignature.Text, "签名已复制到剪贴板");
        ...
        private void BtnClearSignatureData_Click(object? s, EventArgs e) { textSignature.Clear(); SetStatus("签名已清空"); }
        ...
        { if (Clipboard.ContainsText()) { textPrivateKey.Text = Clipboard.GetText().Trim(); _privateKeyPem = textPrivateKey.Text; SetStatus("私钥已从剪贴板粘贴"); } else MessageBox.Show("剪贴板中没有文本内容！", "提示", MessageBoxButtons.OK, MessageBoxIcon.Information); }
        ...
        { if (Clipboard.ContainsText()) { textPublicKey.Text = Clipboard.GetText().Trim(); _publicKeyPem = textPublicKey.Text; SetStatus("公钥已从剪贴板粘贴"); } else MessageBox.Show("剪贴板中没有文本内容！", "提示", MessageBoxButtons.OK, MessageBoxIcon.Information); }

        private void BtnClearPublicKey_Click(object? s, EventArgs e) { textPublicKey.Clear(); _publicKeyPem = string.Empty; SetStatus("公钥已清空"); }

        /// <summary>安全写剪贴板（自动重试3次，避免被其他进程占用）。</summary>
        private bool TrySetClipboardText(string text, string successStatus, string? emptyMessage = null)
        {
            if (string.IsNullOrEmpty(text))
            {
                if (!string.IsNullOrEmpty(emptyMessage)) MessageBox.Show(emptyMessage, "提示", MessageBoxButtons.OK, MessageBoxIcon.Information);
            ...
            {
                try { Clipboard.SetText(text); if (!string.IsNullOrEmpty(successStatus)) SetStatus(successStatus); return true; }
                catch (ExternalException ex) { lastEx = ex; System.Threading.Thread.Sleep(50); }
            }
            MessageBox.Show($"剪贴板操作失败: {lastEx?.Message}\n请稍后再试或关闭可能占用剪贴板的程序。", "复制失败", MessageBoxButtons.OK, MessageBoxIcon.Error);
            SetStatus("复制失败：剪贴板被占用");
            return false;
        }

        private void BtnCopyPrivateKey_Click(object? sender, EventArgs e) => TrySetClipboardText(textPrivateKey.Text, "私钥已复制到剪贴板", "私钥为空，无法复制！");
        private void BtnCopyPublicKey_Click(object? sender, EventArgs e) => TrySetClipboardText(textPublicKey.Text, "公钥已复制到剪贴板", "公钥为空，无法复制！");

        // ==================================================
        //  控件事件
        // ==================================================
        private void ComboOutputFormat_SelectedIndexChanged(object? s, EventArgs e) => RefreshKeyDisplay();
        private void TextPrivateKey_TextChanged(object? s, EventArgs e) { /* 不再自动清空运行结果，保留历史记录 */ }

        // ==================================================
        //  辅助方法
        // ==================================================
        private void SetGenerateResult(string curveName, string status)
            => AppendValidationResult($"{status}\n使用曲线: {EcdsaCurveNames.GetDisplayName(curveName)}", status.Contains('❌') ? Color.Red : Color.Green);
        ...
        {
            if (textKeyResult.Text.Contains("等待操作")) textKeyResult.Clear();
            string detail = message + (string.IsNullOrEmpty(curveName) ? "" : $"\n使用曲线: {EcdsaCurveNames.GetDisplayName(curveName)}");
        ...
        /// <summary>根据输入格式（UTF-8/Base64/Hex）将输入框内容转为字节数组。</summary>
        private byte[] GetEncInputBytes()
        {
            string input = textEncInput.Text.Trim();
            return comboEncInputFormat.SelectedItem?.ToString() switch
            {
        ...
        /// <summary>解析加密结果框中的密文，按输出格式（Base64/Hex）解码。</summary>
        private byte[] GetCipherBytes(string cipherText, string mode)
        {
            if (comboEncOutputFormat.SelectedItem?.ToString() == "Hex")
        ...
        /// <summary>获取/派生对称密钥：手动指定优先，否则从 ECDSA 私钥 HKDF-SHA256 派生。</summary>
        private byte[] GetEncKey()
        {
            if (!string.IsNullOrWhiteSpace(textEncKey.Text))
            {
                byte[] key = Convert.FromHexString(textEncKey.Text.Trim());
                if (key.Length != 32) throw new ArgumentException("对称密钥必须是32字节（256位）HEX字符串");
                return key;
            }
            if (string.IsNullOrWhiteSpace(_privateKeyPem)) throw new InvalidOperationException("请先生成/导入ECDSA私钥，或手动填写对称密钥");
            ...
            byte[] derivedKey = new byte[32]; hkdf.GenerateBytes(derivedKey, 0, derivedKey.Length);
            _derivedEncKey = derivedKey;
            return derivedKey;
        }

        /// <summary>获取 IV/Nonce：手动指定优先，否则随机生成并缓存。</summary>
        private byte[] GetEncIV(string mode, bool forEncryption)
        {
            ...
            if (!string.IsNullOrWhiteSpace(textEncIV.Text))
            {
                ...
                if (!forEncryption) return Convert.FromHexString(currentHex); // 解密直接用 IV 框内容
                string lastHex = _lastEncIV != null ? Convert.ToHexString(_lastEncIV).ToLowerInvariant() : string.Empty;
                if (currentHex != lastHex) return Convert.FromHexString(currentHex); // 用户手动指定了新 IV
            }
            if (!forEncryption) throw new ArgumentException("解密时必须提供 IV（HEX 格式），请从运行结果中复制加密时使用的 IV");
        ...
        #region 加密算法实现（AES-GCM / AES-CBC / ChaCha20）
        ...
        private static byte[] EncryptAesCbc(byte[] plain, byte[] key, byte[] iv)
        {
            ...
            using var encryptor = aes.CreateEncryptor(); return encryptor.TransformFinalBlock(plain, 0, plain.Length);
        }

        private static byte[] DecryptAesCbc(byte[] cipher, byte[] key, byte[] iv)
        {
            using var aes = Aes.Create(); aes.Key = key; aes.IV = iv; aes.Mode = CipherMode.CBC; aes.Padding = PaddingMode.PKCS7;
            using var decryptor = aes.CreateDecryptor(); return decryptor.TransformFinalBlock(cipher, 0, cipher.Length);
        }
        ...
        private static byte[] DecryptChaCha20(byte[] cipherWithTag, byte[] key, byte[] nonce)
        {
            ...
            byte[] tag = cipherWithTag.AsSpan(cipherWithTag.Length - 16, 16).ToArray();
            byte[] plain = new byte[cipher.Length];
            using var chacha = new ChaCha20Poly1305(key); chacha.Decrypt(nonce, cipher, tag, plain);
            return plain;
        }
        #endregion
        ...
        private void BtnEncrypt_Click(object? sender, EventArgs e)
        {
            try
            {
                if (string.IsNullOrWhiteSpace(textEncInput.Text)) { MessageBox.Show("请输入要加密的内容！", "提示", MessageBoxButtons.OK, MessageBoxIcon.Warning); return; }
                ...
                textEncOutput.Text = (comboEncOutputFormat.SelectedItem?.ToString() == "Hex") ? Convert.ToHexString(cipher).ToLowerInvariant() : Convert.ToBase64String(cipher);
                AppendValidationResult($"✅ 加密成功\r\n算法: {mode}\r\nIV: {Convert.ToHexString(iv).ToLowerInvariant()}\r\n长度: {cipher.Length}字节", Color.Green);
                SetStatus("加密完成");
            }
            catch (Exception ex) { AppendValidationResult($"❌ 加密失败: {ex.Message}", Color.Red); SetStatus("加密失败"); }
        }
        ...
        private void BtnDecrypt_Click(object? sender, EventArgs e)
        {
            try
            {
                string cipherSource = string.IsNullOrWhiteSpace(textEncOutput.Text) ? textEncInput.Text.Trim() : textEncOutput.Text.Trim();
                if (string.IsNullOrWhiteSpace(cipherSource)) { MessageBox.Show("请输入要解密的密文！", "提示", MessageBoxButtons.OK, MessageBoxIcon.Warning); return; }
                string mode = comboEncMode.SelectedItem?.ToString() ?? "ECIES (ECDH+AES-GCM)";
            ...
            catch (Exception ex) { AppendValidationResult($"❌ 解密失败: {ex.Message}", Color.Red); SetStatus("解密失败"); }
        }

        private void BtnEncClear_Click(object? sender, EventArgs e)
        {
            textEncInput.Clear(); textEncOutput.Clear(); textEncKey.Clear(); textEncIV.Clear();
            _derivedEncKey = null; _lastEncIV = null;
        }

        private void BtnEncCopy_Click(object? sender, EventArgs e) => TrySetClipboardText(textEncOutput.Text, "加密结果已复制");
        private void BtnEncPaste_Click(object? sender, EventArgs e) { if (Clipboard.ContainsText()) { textEncInput.Text = Clipboard.GetText().Trim(); SetStatus("内容已粘贴到输入框"); } }
        #endregion
        ...
        private void BtnEncryptFile_Click(object? sender, EventArgs e)
        {
            try
            {
                ...
                using var saveDlg = new SaveFileDialog { Title = "保存加密文件", Filter = "加密文件|*.enc", FileName = Path.GetFileNameWithoutExtension(openDlg.FileName) + ".enc" };
                ...
                using var fs = new FileStream(saveDlg.FileName, FileMode.Create); fs.Write(iv, 0, iv.Length); fs.Write(cipher, 0, cipher.Length);
                AppendValidationResult($"✅ 文件加密成功\r\n{Path.GetFileName(openDlg.FileName)} → {Path.GetFileName(saveDlg.FileName)}", Color.Green);
                SetStatus("文件加密完成");
            }
            catch (Exception ex) { AppendValidationResult($"❌ 文件加密失败: {ex.Message}", Color.Red); SetStatus("文件加密失败"); }
        }
        ...
        private void BtnDecryptFile_Click(object? sender, EventArgs e)
        {
            try
            {
                using var openDlg = new OpenFileDialog { Title = "选择要解密的加密文件", Filter = "加密文件|*.enc|所有文件|*.*" };
                if (openDlg.ShowDialog() != DialogResult.OK) return;
                using var saveDlg = new SaveFileDialog { Title = "保存解密文件", Filter = "所有文件|*.*", FileName = Path.GetFileNameWithoutExtension(openDlg.FileName) };
                if (saveDlg.ShowDialog() != DialogResult.OK) return;
                string mode = comboEncMode.SelectedItem?.ToString() ?? "ECIES (ECDH+AES-GCM)";
                byte[] key = GetEncKey();
            ...
            catch (Exception ex) { AppendValidationResult($"❌ 文件解密失败: {ex.Message}", Color.Red); SetStatus("文件解密失败"); }
        }
        #endregion
